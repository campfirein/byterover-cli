...
- Categories: `concurrency`, `llm`, `network`, `task-history`, `updates`. Keys: `agentPool.maxSize`, `agentPool.maxConcurrentTasksPerProject`, `llm.iterationBudgetMs`, `llm.requestTimeoutMs`, `network.host` (host the daemon binds to; default `127.0.0.1`; set to `0.0.0.0` for Docker / LAN), `taskHistory.maxEntries`, `update.checkForUpdates`. Descriptors in `server/core/domain/entities/settings.ts` are discriminated on `type: 'integer' | 'boolean' | 'string'` and reference `src/constants.ts` so a constant change updates the setting's default automatically. `update.checkForUpdates` gates the `update-notifier` init hook and the `block-autoupdate-when-off` init hook (which rejects `brv update` when the user has opted out). `network.host` is read once at daemon startup by `bootstrapSettings` and threaded to both `SocketIOTransportServer.start(port, host)` and `WebUiServer.start(port, host)`. Agent process consumes settings via `agent/infra/settings/agent-settings-snapshot.ts` forwarded by `server/infra/daemon/agent-process.ts`
...
- `BRV_TRANSPORT_HOST` — override the host the daemon (Socket.IO + WebUI HTTP) binds to. Default `127.0.0.1`; set to `0.0.0.0` for Docker / LAN. Takes precedence over `network.host` in `settings.json`. Important: env must be set on the command that **spawns** the daemon, not on `brv restart` (restart only kills; the next `brv <cmd>` is the spawn trigger and inherits the env). For persistent setups use `export BRV_TRANSPORT_HOST=…` in shell rc / Docker `ENV` / systemd `Environment=`
