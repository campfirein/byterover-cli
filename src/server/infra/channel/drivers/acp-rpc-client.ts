...
/** Carries a JSON-RPC error response's `code`/`message`/`data` to the caller. */
...
/** JSON-RPC 2.0 peer: correlates calls, routes notifications + server requests. */
export class AcpRpcClient {
  ...
  public constructor(transport: AcpRpcTransport) {
  ...
  /** Sends a request and resolves with its result (rejects on error/close). */
  ...
  /**
   * Pushes raw bytes into the decoder. Useful when the caller drives the
  ...
  /** Registers a handler for an inbound server-initiated request method. */
