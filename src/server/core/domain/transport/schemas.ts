...
import {CliRequestBaseSchema} from '../../../../shared/analytics/cli-metadata-schema.js'
 ...
 *
 * Merged with `CliRequestBaseSchema` so proactive Socket.IO clients (oclif,
 * TUI, MCP, webui) can attach an optional `cli_metadata` block describing the
 * invocation. Daemon-only consumers (idle dispatch, agent fork) leave it out.
 */
...
// Analytics wire shape moved to src/shared/transport/events/analytics-events.ts
// so both shared callers (emitAnalytics) and the daemon handler reference a
// single Zod-derived type. Keeping a server-side duplicate caused a drift
// risk between the manual interface and the schema-derived type.
