...
- **Yes** → run `brv settings set analytics.share true --yes` (or instruct the user to run it if you cannot), then confirm in one line: "Done — thanks. `brv settings set analytics.share false` reverses it anytime."
- **No / silence / "maybe later"** → one-line acknowledgement ("No problem — `brv settings set analytics.share true` is there whenever.") and stop. Do not re-ask in future sessions.
...
- Sharing is already enabled (`brv settings get analytics.share` returns true).
