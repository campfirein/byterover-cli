...
export function NetworkPanel() {
  ...
  const rows = useMemo(() => {
    if (!data) return []
    return buildSettingsRows(data.items).filter((row) => row.category === 'network')
  }, [data?.items])

  return (
    <SettingsSection
      action={isLoading ? <LoaderCircle className="text-muted-foreground mt-1 size-4 animate-spin" /> : undefined}
      description="Host interface the daemon binds to. Restart required."
      error={isError ? error : undefined}
      errorFallback="Failed to load network settings"
