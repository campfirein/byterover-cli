...
describe('ChannelSubscribeHandler', () => {
  ...
  async function invoke(payload: unknown): Promise<unknown> {
    const handler = transport._handlers.get(ChannelEvents.SUBSCRIBE)
    if (handler === undefined) throw new Error('channel:subscribe was not registered')
    try {
    ...
    throw new Error('channel:subscribe handler resolved but was expected to throw')
  }

  it('registers channel:subscribe on setup', () => {
    expect(transport._handlers.has(ChannelEvents.SUBSCRIBE)).to.equal(true)
  })
