...
// M0-1 — pure FSM for the channel turn/delivery lifecycle.
//
// M0 only EXERCISES the passive-turn transitions, but the FSM defines the FULL
// transition table so M2 dispatch + M3 multi-agent fan-out land additively
// without re-touching this module.
describe('TurnStateMachine', () => {
  describe('turn-level transitions', () => {
    it('accepts pending → completed (passive finalisation)', () => {
      expect(isLegalTurnTransition('pending', 'completed')).to.equal(true)
    })

    it('accepts pending → dispatched (mention dispatches resolved mentions)', () => {
    ...
    it('rejects transitions out of terminal turn states', () => {
