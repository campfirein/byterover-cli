// Shared NDJSON / JSON-RPC plumbing for the mock-ACP fixtures.
//
...
// JSON-RPC error response. AUTH_REQUIRED fixtures use this to emit
// `{code: -32000, data: {authMethods: [...]}}`.
...
export const start = (behaviour) => {
  ...
  rl.on('line', (line) => {
    ...
    if (msg.method === 'session/new') {
      // Onboard probing reads `session/new` outcomes when classifying driver
      // class. Fixtures may supply a custom handler that returns either a
      // result object OR an Error to surface a session failure.
