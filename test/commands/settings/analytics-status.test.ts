...
/**
 * Smoke coverage for the post-M16.4 surface of `analytics.status`.
 *
 * The text + JSON parity and the snapshot composition are exercised by:
 *   - test/unit/shared/utils/format-analytics-status.test.ts
 *   - test/unit/server/infra/analytics/build-status-snapshot.test.ts
 ...
 * the unified `settings:get` transport event — i.e. the legacy
 * `brv analytics status` deletion does not leave the value unreachable
 * via the CLI.
 */
describe('brv settings get analytics.status (M16.4 smoke)', () => {
  let config: Config
  ...
  it('routes to the SettingsEvents.GET transport event with key=analytics.status', async () => {
    const command = new TestableSettingsGet(['analytics.status'], mockConnector, config)
    ...
    expect(calls[0].args[1]).to.deep.equal({key: 'analytics.status'})
  })
})
